...
module Nav25 =
    ...
    let root model dispatch: ReactElement =
        let onEnter msg: DOMAttr =
            function
            | (ev: Browser.Types.KeyboardEvent) when ev.keyCode = 13. -> dispatch msg
            | _ -> ()
            |> OnKeyDown

        let viewFilter filter isFilter filterText filterPath =
            if isFilter then
                span [ ClassName "mr-3" ] [ str filterText ]
            else
                a
        ...
        let viewFilters filter =
            div []
                [ viewFilter All (filter = All) "All" "navigationtodos25"
                  ...
                  viewFilter Incomplete (filter = Incomplete) "incomplete" "navigationtodos25/incomplete"

                  viewFilter Completed (filter = Completed) "completed" "navigationtodos25/completed" ]
        ...
        let viewNormalTodo (todo: Todo): ReactElement =
            div [ ClassName "card" ]
                [ div [ ClassName "card-block" ]
                      [ input
                          [ OnClick(fun _ -> dispatch <| ToggleTodo todo.Id)
                            Type "checkbox"
                            Checked todo.Completed
                            ClassName "mr-3" ]

                        span
                            [ OnDoubleClick(fun _ -> dispatch <| Edit(todo.Id, todo.Text))
                              Style
                                  [ TextDecoration
                                      (if todo.Completed then "line-through"
                                       else "none") ] ] [ str todo.Text ]
                        span
                            [ OnClick(fun _ -> dispatch <| RemoveTodo todo.Id)
        ...
        div [ ClassName "col-12 col-sm-6 offset-sm-3" ]
            [ div [ ClassName "row" ]
                  [ div [ ClassName "col-9" ]
                    ...
                    div [ ClassName "col-3" ]
                        [ button
                            [ OnClick(fun _ -> dispatch GenerateTodoId)
...
module Nav25b =
    ...
    let viewFilter (filter: Filter) (isFilter: bool) (fitlerText: string) (filterPath: string): ReactElement =
        if isFilter then
            span [ ClassName "mr-3" ] [ str fitlerText ]
        else
            a
                [ ClassName "text-primary mr-3"
                  Href <| "#" + filterPath
    ...
    let viewFilters (filter: Filter): ReactElement =
        div []
            [ viewFilter All (filter = All) "All" "navigationtodos25b"
        ...
        div [ ClassName "col-12 col-sm-6 offset-sm-3" ]
            [ div [ ClassName "row" ]
              ...
              div []
                  (model.Todos
                   |> filterTodos model.Filter
                   |> List.map (fun todo -> viewTodo model.Editing todo dispatch)) ]
